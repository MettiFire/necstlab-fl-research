...
import os
import sys
# Fix: aggiungi la cartella custom al PYTHONPATH
import os as _os
...
class XGBExecutor(Executor):
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal) -> Shareable:
        ...
        self.log_info(fl_ctx, f"[TIMING_CLIENT] R{round_num} C{self.client_id} t2_recv={timestamp_t2_recv:.6f}")
        ...
        if timestamp_t1_send > 0:
            ...
            self.log_info(fl_ctx, f"[TIMING_CLIENT] R{round_num} C{self.client_id} latenza_up={latenza_up_ms:.1f}ms")
        ...
        self._save_local_metrics(metriche, fl_ctx)
    ...
    def _save_local_metrics(self, metriche: dict, fl_ctx=None) -> None:
        try:
            site_name = fl_ctx.get_identity_name() if fl_ctx else str(self.client_id)
            percorso_timing = Path(f"/tmp/timing_nvflare_client_{site_name}.json")
            ...
            self.log_warning(fl_ctx, f"Failed to save local metrics: {e}")
